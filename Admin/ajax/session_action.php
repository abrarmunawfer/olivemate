Here is the fully updated PHP code. The IP Geolocation block has been replaced with a **cURL implementation** that specifically fetches **latitude and longitude** and saves them as a comma-separated string (`lat,lon`) to the `location` column, as requested. This is generally more reliable than `file_get_contents()` and provides precise coordinates.
...
                    
                    // Default location saves descriptive text for failure or 'Localhost'
                    $location = "Location data unavailable"; 

                    // --- IP Geolocation (UPDATED TO USE cURL AND SAVE LAT/LON) ---
                    try {
                        if ($ip_address != '127.0.0.1' && $ip_address != '::1') {
                            // Request status, lat, and lon fields
                            $api_url = "http://ip-api.com/json/{$ip_address}?fields=status,message,lat,lon";
...
                                    
                                    // Store the raw Lat,Lon as a comma-separated string
                                    $lat = number_format((float)($geo_data->lat ?? 0), 6, '.', '');
                                    $lon = number_format((float)($geo_data->lon ?? 0), 6, '.', '');
                                    $location = substr("{$lat},{$lon}", 0, 255); // Ensure it's truncated for DB
                                    
                                } elseif ($geo_data && property_exists($geo_data, 'status') && $geo_data->status == 'fail') {
                                    // Log if the API explicitly failed (e.g., rate limit, reserved IP)
                                    error_log("IP-API Status Fail: " . ($geo_data->message ?? 'Unknown reason') . " for IP: " . $ip_address);
                                }
                            }
                        } elseif ($ip_address == '127.0.0.1' || $ip_address == '::1') {
                            // Use a dummy value for localhost
